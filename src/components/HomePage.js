...
const HomePage = ({ logout }) => {
  ...
  const [justOpen, setJustOpen] = React.useState(true);

  const loadFragment = (name) => {
    setFragment(name);
  };

  const closeJustOpen = () => {
    setTimeout(() => setJustOpen(false), 3000);
  };
  ...
  return (
    <Box sx={{ display: "flex" }}>
        ...
        <Box sx={{ overflow: "auto" }}>
          <List>
            <SidebarOptions
              Icon={HomeIcon}
              text="Home"
              onChange={() => loadFragment("Home")}
              active={fragment === "Home" ? true : false}
            />
            <SidebarOptions
              Icon={CategoryIcon}
              text="Categories"
              onChange={() => loadFragment("Categories")}
              active={fragment === "Categories" ? true : false}
            />
            <SidebarOptions
              Icon={PhonelinkLockIcon}
              text="Products"
              onChange={() => loadFragment("Add Products")}
              active={fragment === "Add Products" ? true : false}
            />
            <SidebarOptions
              Icon={ShoppingCartIcon}
              text="Orders"
              onChange={() => loadFragment("Orders")}
              active={fragment === "Orders" ? true : false}
            />
            <SidebarOptions Icon={LogoutIcon} text="Logout" onChange={logout} />
          </List>
          <Divider />
        </Box>
      </Drawer>
      <Box component="main" sx={{ flexGrow: 1 }}>
        ...
        {fragment == "Categories" ? <ManageCategories /> : ""}
        {fragment == "Add Products" ? <ManageProduct /> : ""}
